"""TimeSeriesX.py, author=LoganF (not really, just saving my personal add-ins...)
A script containing TimeSeriesX but also Logan's handy TimeSeriesX.concat function
"""
import json
import warnings
...
class TimeSeriesX(xr.DataArray):
    ...
    @classmethod
    def concat(cls, ts_list, dim, *args, **kwargs):
        ...
        dims_to_check = np.unique([ts.dims for ts in ts_list])
        # Figure out which ones could have messed up indexing
        needs_numpy = np.unique([[dims for dims in dims_to_check
                                  if isinstance(ts[dims].data, np.ndarray)]
                                 for ts in ts_list])
        # Create a dictionary that holds concatenated dimensions
        replacement = {dims: np.unique(np.concatenate(
            [ts['{}'.format(dims)].data for ts in ts_list])).view(np.recarray)
                       for dims in needs_numpy}

        ts = xr.concat(objs=ts_list, dim=dim, *args, **kwargs)

        # Replace it such that you still have indexing!!
        for k, v in enumerate(sorted(replacement)):
            ts['{}'.format(v)].data = replacement[v]
